...
private:
    ...
    static constexpr const char* kRepo = "localteampartners/sappkeys";
    ...
    void runCheck()
    {
        ...
        auto stream = api.createInputStream(
            juce::URL::InputStreamOptions(juce::URL::ParameterHandling::inAddress)
                .withExtraHeaders("User-Agent: SappKeys-Updater")
        ...
        if (!sapp::keys::isNewerVersion(tag.toRawUTF8(), JucePlugin_VersionString))
    ...
    void runInstall()
    {
        ...
        for (const auto& dir : tempDir.findChildFiles(juce::File::findDirectories, false))
            if (dir.getFileName().startsWith("SappKeys")) { stage = dir; break; }
    ...
    {
        const auto newVst = stage.getChildFile("SappKeys.vst3");
        ...
        if (target == juce::File())
            target = juce::File::getSpecialLocation(juce::File::windowsLocalAppData)
                         .getChildFile("Programs/Common/VST3/SappKeys.vst3");

        const auto parked = target.getSiblingFile("SappKeys.vst3.old-" + tag);
    ...
    bool installMac(const juce::File& stage, const juce::String& tag, juce::String& note)
    {
        ...
        install("SappKeys.vst3", home.getChildFile("Library/Audio/Plug-Ins/VST3"));
        install("SappKeys.component", home.getChildFile("Library/Audio/Plug-Ins/Components"));

        // Standalone: stage the new app next to the current one.
        if (juce::JUCEApplicationBase::isStandaloneApp()) {
            const auto newApp = stage.getChildFile("SappKeys.app");
            const auto currentApp =
                juce::File::getSpecialLocation(juce::File::currentApplicationFile);
            if (newApp.exists() && currentApp.getFileName().endsWith(".app")) {
                const auto parked = currentApp.getSiblingFile("SappKeys.app.old");
                parked.deleteRecursively();
